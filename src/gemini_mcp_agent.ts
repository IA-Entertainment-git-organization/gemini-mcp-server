...
class MCPAgent {
  ...
  constructor(apiKey: string, serverParams: MCPServerParameters) {
    this.apiKey = apiKey;
    this.apiClient = new GeminiApiClient(apiKey, MODEL_ID);
  ...
  async processUserInput(
    ...
    try {
      ...
      const response = await this.makeGeminiRequest(contents);
      ...
      for (const part of response.candidates[0].content.parts as ExtendedPart[]) {
        if (part.functionCall) {
          ...
          // Make another direct API call for follow-up
          const followUpResponse = await this.makeGeminiRequest(contents);
  ...
  // Method to directly call the Gemini API
  private async makeGeminiRequest(contents: Content[]): Promise<GenerateContentResult> {
    try {
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${MODEL_ID}:generateContent?key=${this.apiKey}`;
      ...
      return await response.json() as GenerateContentResult;
    } catch (error) {
